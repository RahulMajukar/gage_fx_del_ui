...
const fetchAllCalibrationHistory = async () => {
  try {
    const response = await api.get(`${CALIBRATION_BASE}/history`);
...
const CalibrationDashboard = () => {
  ...
  const loadCalibrationHistory = async () => {
    try {
      ...
      const historyData = await fetchAllCalibrationHistory();
      setCalibrationHistory(historyData);

      // Also update the count
      const uniqueCalibrations = historyData.filter((item, index, self) =>
