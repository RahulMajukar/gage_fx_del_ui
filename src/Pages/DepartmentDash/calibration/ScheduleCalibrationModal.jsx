...
const ScheduleCalibrationModal = ({ isOpen, onClose, selectedGage, onSubmit }) => {
  const [formData, setFormData] = useState({
    scheduledDateTime: dayjs().add(1, 'day').set('hour', 9).set('minute', 0),
    priority: 'medium',
    assignedTo: '',
    laboratory: '',
    estimatedDuration: 2,
  ...
  const generateEmailContent = () => {
    const formattedDate = formData.scheduledDateTime.format('dddd, MMMM D, YYYY');
    const formattedTime = formData.scheduledDateTime.format('h:mm A');
...
Quality Assurance Department`;
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate form
    if (!formData.scheduledDateTime || !formData.scheduledDateTime.isValid()) {
    ...
    try {
      const payload = {
        gageId: selectedGage?.id,
        scheduledDate: formData.scheduledDateTime.format('YYYY-MM-DD'),
        scheduledTime: formData.scheduledDateTime.format('HH:mm'),
        priority: formData.priority,
        assignedTo: formData.assignedTo.trim(),
        laboratory: formData.laboratory,
        estimatedDuration: parseInt(formData.estimatedDuration) || 2,
      ...
      const res = await fetch(`http://localhost:8080/api/calibration-manager/gages/${selectedGage?.id}/schedule`, {
        ...
        headers: {
          "Content-Type": "application/json",
          "User-ID": "1",
          "User-Email": localStorage.getItem("email") || "test@swajyot.co.in"
        },
        body: JSON.stringify(payload)
      });
      ...
      // Create calendar event
      const startDateTime = formData.scheduledDateTime.toDate();
      const endDateTime = formData.scheduledDateTime.add(formData.estimatedDuration, 'hour').toDate();
              ...
              <div className="space-y-4">
                ...
                <div className="mb-4">
                  ...
                  <DateTimePicker
                    ...
                    onChange={(newValue) => handleChange('scheduledDateTime', newValue)}
                    minDateTime={dayjs()}
                    ampm={true}
                    disablePast
                    ...
                    slotProps={{
                      textField: {
                        ...
                        InputProps: {
                          className: "px-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500",
                        }
                      },
                      popper: {
                        className: "z-[100]",
