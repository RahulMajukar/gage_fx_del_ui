...
const GageDrawer = ({
  ...
  const handleInwardSubmit = async (formData, fileSummary) => {
    try {
      ...
      if (response.ok) {
        ...
        resultText = `Inward processed successfully with ${totalFiles} file(s). Gage inward processed successfully and status set to ACTIVE.`;
        // if (data && data.message) resultText += ` Gage inward processed successfully and status set to ACTIVE.`;
        Swal.fire({
          title: "Success!",
          text: resultText,
          icon: "success",
  ...
  const renderDetailView = () => {
    ...
    const isDueSoon = !isOverdue && daysUntilDue !== null && daysUntilDue <= 15;
        ...
        <div className={`rounded-2xl p-8 mb-8 ${
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {isDueSoon && (
                <div className="bg-orange-100 px-4 py-2.5 rounded-xl flex items-center gap-2 border border-orange-200 animate-pulse">
                  <AlertTriangle className="h-4 w-4 text-orange-600" />
                  <span className="font-bold text-orange-700">DUE SOON</span>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="bg-white/70 backdrop-blur-lg rounded-2xl p-6 border border-gray-200 shadow-sm">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className={`font-bold ${isOverdue ? 'text-red-600' : isDueSoon ? 'text-orange-600' : 'text-gray-900'}`}>
                ...
                {daysUntilDue !== null && (
                  <div>
                    ...
                    <p className={`font-bold ${isOverdue ? 'text-red-600' : isDueSoon ? 'text-orange-600' : 'text-emerald-600'}`}>
                      {isOverdue
                ...
                {isDueSoon && (
                  <div className="mt-4 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                    <div className="flex items-center gap-2 mb-1">
                      <AlertTriangle className="h-4 w-4 text-orange-600" />
                      <p className="text-sm font-semibold text-orange-700">Urgent Action Required</p>
                    </div>
                    <p className="text-xs text-orange-600">
                      This instrument requires immediate scheduling for calibration.
                    </p>
                  </div>
                )}
              </div>
                ...
                {selectedFilter === 'total' && hasGages && (
                  <div className="mt-4 flex flex-wrap items-center gap-4">
                    <div className="flex items-center gap-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-white font-medium">
                          Overdue: {filteredGages.filter(g => {
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-white font-medium">
                          Due Soon (≤15 days): {filteredGages.filter(g => {
                            const days = g.dueDate ? daysUntil?.(g.dueDate) : null;
                            return days !== null && days >= 0 && days <= 15;
                          }).length}
                        </span>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-white font-medium">
                          On Track: {filteredGages.filter(g => {
                            const days = g.dueDate ? daysUntil?.(g.dueDate) : null;
                    ...
                    <button
                      onClick={() => {
                        // Show urgent items
                        const urgentItems = filteredGages.filter(g => {
                          const days = g.dueDate ? daysUntil?.(g.dueDate) : null;
                          return days !== null && days <= 15;
                        });
                        // You might want to implement filtering logic here
                        console.log('Urgent items:', urgentItems);
                      }}
                      className="ml-auto px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg text-sm font-medium transition-colors flex items-center gap-2"
                    >
                          ...
                          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            ...
                            <div className="flex flex-col items-end gap-2">
                              <div className="text-right">
                                ...
                                {/* Color code the due date text */}
                                <p className={`font-bold ${isOverdue ? 'text-red-600' : dueSoon ? 'text-orange-600' : 'text-gray-900'}`}>
                                  {g.dueDate || '—'}
                                </p>
