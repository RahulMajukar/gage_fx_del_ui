        ...
        <div className="flex flex-col h-full p-4">
          ...
          <nav className="flex-1 overflow-y-auto space-y-1">
            {!isCalibrationManager && user.role && (
              <NavLink
                to={user.role === "IT_ADMIN" ? "/dashboard/it-admin" : `/dashboard/${user.role.toLowerCase()}`}
                onClick={closeAll}
                className={({ isActive }) =>
                  `flex items-center gap-3 py-2.5 px-3 rounded-lg text-sm font-medium transition-all ${
                    isActive ? "bg-[#004a80] text-white shadow-md" : "text-white hover:text-white hover:bg-[#004a80]"
                  }`
                }
              >
                {/* ✅ Changed from LayoutDashboard to Home - no longer looks like QR code */}
                <Home size={16} />
                Dashboard
              </NavLink>
            )}
                ...
                {openSections.calibration && (
                  <div className="mt-1 space-y-1">
                    <NavLink
                      ...
                      onClick={closeAll}
                      className={({ isActive }) =>
                        `flex items-center gap-2 py-2 px-6 rounded text-sm transition-all ${
                          isActive ? "bg-[#004a80] text-white" : "text-white hover:text-white hover:bg-[#004a80]"
                        }`
                      }
                    >
          ...
          <div className="pt-4 mt-4 border-t border-[#004a80]">
            ...
            <button
              ...
              className={`flex items-center justify-between w-full py-2 px-3 rounded-lg text-sm transition-all ${
                showReallocFeature ? "text-white hover:bg-[#004a80] cursor-pointer" : "text-white/60 cursor-not-allowed"
              }`}
            >
              <div className="flex items-center gap-2">
                <Bell size={16} />
          ...
          <div className="mt-auto pt-4">
            <div className="flex items-center gap-3 p-3 rounded-xl bg-[#004a80]">
              <div className="w-10 h-10 rounded-full overflow-hidden">
                {userInfo.profileImage ? (
                  <img src={userInfo.profileImage} alt="avatar" className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full bg-white flex items-center justify-center text-[#005797] font-bold">
            ...
            <button onClick={handleLogout} className="flex items-center gap-2 w-full py-2 px-3 mt-2 rounded text-sm text-white hover:text-red-400 hover:bg-red-600/10 transition-all duration-200">
              <LogOut size={16} />
              <span>Sign Out</span>
            </button>
          </div>
