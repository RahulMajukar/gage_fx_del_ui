...
import { X, Calendar, Clock, MapPin, Users, Tag, Edit, Trash2, CheckCircle, XCircle, AlertCircle, Mail } from 'lucide-react'
...
export default function EventModal({
  ...
  // FIX: Don't show accept/decline buttons for owners
  const showResponseButtons = canRespond && event.userRelationship !== 'owner'
  ...
  const attendees = Array.isArray(event.attendees) 
    ? event.attendees.map(formatAttendee)
    : []
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center space-x-2 flex-wrap">
                ...
                {event.userRelationship === 'attendee' && (
                  <span className="text-xs px-3 py-1 rounded-full font-medium bg-green-100 text-green-700 border border-green-300">
                    👤 Attendee
                  </span>
                )}
              </div>
        ...
        <div className="p-6 overflow-y-auto max-h-[60vh]">
          ...
          <div className="space-y-4">
            ...
            {/* Attendees with detailed status - FIXED: Proper email display */}
            {attendees.length > 0 && (
              <div className="flex items-start space-x-3">
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-semibold text-sm">
                            {(attendee.name || attendee.email || 'U').charAt(0).toUpperCase()}
                          </div>
                          <div>
                    ...
                    <div className="flex items-center justify-between text-xs text-gray-600">
                      <div className="flex items-center space-x-4">
                        <span className="flex items-center">
                          ...
                          {attendees.filter(a => a.status?.toUpperCase() === 'ACCEPTED').length} Accepted
                        </span>
                        <span className="flex items-center">
                          <XCircle className="h-3 w-3 text-red-600 mr-1" />
                          {attendees.filter(a => a.status?.toUpperCase() === 'DECLINED').length} Declined
                        </span>
                        <span className="flex items-center">
                          <Clock className="h-3 w-3 text-yellow-600 mr-1" />
                          {attendees.filter(a => a.status?.toUpperCase() === 'PENDING' || !a.status).length} Pending
                        </span>
                      </div>
                    </div>
                  </div>
        ...
        {/* Footer with Action Buttons - FIXED: Hide accept/decline for owners */}
        <div className="p-6 border-t border-gray-200 bg-gray-50">
          <div className="flex items-center justify-between flex-wrap gap-3">
            {/* Accept/Decline Buttons (for attendees with pending status, not owners) */}
            {showResponseButtons && (
              <div className="flex items-center space-x-3">
                <button
            ...
            {!showResponseButtons && event.userRelationship !== 'owner' && (
              <button
                onClick={onClose}
